...
class OLEDDisplay:
    ...
    def show_text(self, text, x=0, y=0, clear_first=True):
        ...
        try:
            with self._lock:
                ...
                lines = []
                current_line = ""
                max_chars = 18  # Approximate chars per line at font size 10
    ...
    def show_text_streaming(self, text):
        """Display text for streaming LLM output — shows last visible portion."""
        try:
            with self._lock:
                self._draw.rectangle((0, 0, WIDTH, HEIGHT), fill=0)

                # Break text into displayable lines
                max_chars = 18
                lines = []
                current_line = ""
                ...
                # Show last 5 lines (scrolling effect)
